...
export default function GroupTableCard({ groupLabel, teams, existingBet, onBet }: Props) {
  ...
  // Available teams for each dropdown (exclude already selected)
  function availableFor(position: 'first' | 'second' | 'third') {
    const selected = { first, second, third }
    return teams.filter(t => {
      if (position === 'first') return t.abbr !== selected.second && t.abbr !== selected.third
      if (position === 'second') return t.abbr !== selected.first && t.abbr !== selected.third
      return t.abbr !== selected.first && t.abbr !== selected.second
    })
  }
        ...
        <div className="p-4">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-green/10 text-green">
                ✅ Aposta feita
              </span>
            </div>
          </div>
          <div className="space-y-2">
            {positions.map(({ pos, abbr, color }) => {
              const team = getTeam(abbr)
              return (
                <div key={pos} className="flex items-center gap-3 bg-dark-3 rounded-xl px-3 py-2">
    ...
    <div className="card overflow-hidden hover:border-white/20 transition-all">
      ...
      <div className="p-4">
        <div className="flex justify-between items-center mb-3">
          ...
          <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-white/[0.06] text-muted">
            🕐 Aberto
          </span>
        </div>
          ...
          {positions.map(({ label, value, setter, color, position }) => (
            <div key={label} className="flex items-center gap-2">
              ...
              >
                ...
                {availableFor(position).map(t => (
                  <option key={t.abbr} value={t.abbr}>
                    {t.flag} {t.name} ({t.abbr})
                  </option>
                ))}
              </select>
            </div>
